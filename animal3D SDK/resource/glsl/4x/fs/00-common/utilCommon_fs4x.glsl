...
float attenuation2(in float distSq, in float lightRadiusInvSq)
{
	float x = 1.0 - distSq * lightRadiusInvSq;
	return max(0.0, x);
}

float attenuation2(in float dist, in float distSq, in vec4 lightRadiusInfo)
{
	return attenuation2(distSq, lightRadiusInfo[iValInvSq]);
}
	...
	in vec4 lightPos, in vec4 lightRadiusInfo, in vec4 lightColor)
{
	...
	vec4 color = fragColor * lightColor * attenuation2(dist, distSq, lightRadiusInfo);
	diffuseColor = color * kd;
	specularColor = color * ks;
}
